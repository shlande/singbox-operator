...
func (r *ProxyRouteReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	defer func() {
		...
		if reconcileErr != nil {
			result = "error"
			metrics.ReconcileErrorsTotal.WithLabelValues("proxyroute", "reconcile_error").Inc()
		}
		metrics.ReconcileDurationSeconds.WithLabelValues("proxyroute", result).Observe(time.Since(start).Seconds())
	}()
